...
############################
STACK_TEX="$RESULT_DIR/all_stack_plots.tex"
...
for i in $(seq 0 30); do
    echo -n "    \includegraphics[width=0.11\\textwidth]{../plots/stack_ep_bin_$i.png}" >> "$STACK_TEX"
    if (( (i + 1) % 8 == 0 )); then
        echo " \\\\" >> "$STACK_TEX"
    else
        echo " %" >> "$STACK_TEX"
...
cd "$RESULT_DIR"
pdflatex -interaction=nonstopmode all_stack_plots.tex > /dev/null

if [ -f all_stack_plots.pdf ]; then
    echo "[SUCCESS] PDF luotu: results/all_stack_plots.pdf"
else
...
cp all_stack_plots.pdf combined_report.pdf