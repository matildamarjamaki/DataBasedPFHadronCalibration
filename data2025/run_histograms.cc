...
std::string NormalizeInputPath(const std::string &s) {
    if (s.rfind("/store/", 0) == 0) {
        // Kaikki /store-polut avataan globaalin redirectorin kautta
...
void run_histograms(const char* listfile, const char* tag, const char* outpath) {
    ...
    while (std::getline(infile, line)) {
        ...
        if (!norm.empty()) files.push_back(norm);
    }
    ...
    //auto df_base = df; // käytä tätä eteenpäin
...
// -------------------------------------
// PÄÄOHJELMA (Condor-jobille tai paikallisesti)
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <listfile> <tag> <output.root>\n";
        return 1;
    }
    ...
    run_histograms(listfile, tag, outpath);
    return 0;
}